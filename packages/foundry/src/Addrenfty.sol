...
contract Addrenfty is ERC721("AddreNFTy", "ADNFT") {
    ...
    function svgTextLine(uint256 x, uint256 y, string memory str) pure internal returns (string memory) {
        return string.concat(
            '<text x="', x.toString(), '" y="',y.toString(),'" font-family="Courier New" font-size="36" text-anchor="middle" filter="url(#a)">',
            str, '</text>');
    }

    function getAddressChunk(string memory str, uint256 start) pure internal returns (string memory chunk) {
        chunk = string.concat(slice(str, start, 2));
        for(uint i = start+2; i < start+10; i+=2) {
    ...
    function slice(string memory _str, uint _start, uint _length) internal pure returns (string memory) {
